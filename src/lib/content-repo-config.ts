...
const splitUrlPath = (value: string) =>
  trimSlashes(String(value || "").split(/[?#]/)[0])
    .split("/")
    .filter(Boolean);
...
export const normalizeContentRepoPath = (filePath: string) =>
...
export const stripContentRepoBasePath = (filePath: string) => {
  ...
  if (normalized === CONTENT_REPO_BASE_PATH) {
    return "";
  }
...
export const resolveContentRepoPath = (filePath: string) => {
  ...
  return [CONTENT_REPO_BASE_PATH, normalized]
    .filter(Boolean)
    .join("/");
};
...
export const getContentRepoPathCandidates = (filePath: string) => {
  ...
  const candidates = [
    [CONTENT_REPO_BASE_PATH, normalized]
      .filter(Boolean)
      .join("/"),
  ].filter(Boolean);
