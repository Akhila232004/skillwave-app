...
const ACTIVE_LIBRARY_USER_KEY =
  "tinitiate_library_active_user";

const FAVORITES_STORAGE_PREFIX =
  "favorite_topics_";

const FAVORITES_STORAGE_FALLBACK =
  "favorite_topics";

export const normalizeLibraryUserKey = (
  value: unknown
) =>
  String(value || "")
    .trim()
...
const normalizeFavoriteTopic = (
  value: unknown
): SavedFavoriteTopic | null => {
  if (
    !value ||
    typeof value !== "object"
  ) {
...
const resolveLibraryUserKey = (
  ...
  return normalizeLibraryUserKey(
...
const getFavoritesStorageKey = (
  ...
  const scope =
    resolveLibraryUserKey(
      accountKey
    );

  return scope
    ? `${FAVORITES_STORAGE_PREFIX}${scope}`
    : FAVORITES_STORAGE_FALLBACK;
};

const sortFavorites = (
  topics: SavedFavoriteTopic[]
) =>
  [...topics].sort(
    (a, b) => {
      if (
...
export const getFavoriteIdentity = (
  item: Pick<
    SavedFavoriteTopic,
    "slug" | "kind"
  >
...
export const setActiveLibraryUserKey = (
  accountKey?: string
) => {
  if (
    typeof window ===
...
export const mergeFavoriteTopics = (
  ...lists: Array<
    SavedFavoriteTopic[] |
      undefined
  >
...
export const readFavoriteTopics = (
  accountKey?: string
): SavedFavoriteTopic[] => {
  if (
    typeof window ===
...
export const writeFavoriteTopics = (
  topics: SavedFavoriteTopic[],
  accountKey?: string
) => {
  if (
...
export const upsertFavoriteTopic = (
